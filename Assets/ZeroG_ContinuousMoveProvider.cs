...
namespace UnityEngine.XR.Interaction.Toolkit.Locomotion.Movement
{
    ...
    /// Modified for zero-gravity flight with independent acceleration (inertia-like behavior).
    /// </summary>
    /// <seealso cref="LocomotionProvider"/>
    /// <seealso cref="ContinuousTurnProvider"/>
    [AddComponentMenu("XR/Locomotion/ZeroG Continuous Move Provider", 11)]  // MODIFIED: Updated menu name
    public partial class ZeroG_ContinuousMoveProvider : LocomotionProvider, IGravityController  // MODIFIED: Renamed class
    {
        [SerializeField]
        ...
        [SerializeField]
        [Tooltip("Controls whether to enable flying (unconstrained movement). This overrides the use of gravity. Set to true for zero-gravity.")]
        bool m_EnableFly = true;  // MODIFIED: Default to true for zero-gravity
        /// <summary>
        ...
        // NEW: Acceleration rate for building velocity over time (independent from direct movement)
        [SerializeField]
        [Tooltip("Rate at which acceleration builds velocity (units/second). Higher = more responsive.")]
        float m_AccelerationRate = 5f;
        public float accelerationRate
        {
            get => m_AccelerationRate;
            set => m_AccelerationRate = value;
        }

        // NEW: Drag factor for velocity decay (0-1, where 1 = no drag, 0.9 = slow decay)
        [SerializeField]
        [Tooltip("Drag factor for velocity decay (0-1). Lower = more inertia.")]
        float m_Drag = 0.95f;
        public float drag
        {
            get => m_Drag;
            set => m_Drag = Mathf.Clamp01(value);  // Clamp to 0-1
        }
        ...
        // NEW: Persistent velocity for inertia (accumulates acceleration, decays with drag)
...
#pragma warning restore CS0618
        ...
        protected void OnEnable()
        {
            ...
            m_CurrentVelocity = Vector3.zero;  // NEW: Reset velocity on enable
        }
        ...
        protected void Update()
        {
            ...
            if (input != Vector2.zero || m_GravityDrivenVelocity != Vector3.zero || m_InAirVelocity != Vector3.zero || m_CurrentVelocity != Vector3.zero)  // MODIFIED: Include currentVelocity check
        ...
        /// MODIFIED: Now applies acceleration to persistent velocity for inertia.
        /// </summary>
        ...
        {
            if (input == Vector2.zero && m_InAirVelocity == Vector3.zero && m_CurrentVelocity == Vector3.zero)  // MODIFIED: Check currentVelocity
                return Vector3.zero;
            ...
            if (m_EnableFly)
            {
                ...
                // NEW: Apply acceleration to currentVelocity (inertia), then decay with drag
                Vector3 desiredAcceleration = combinedMove * m_AccelerationRate;
                m_CurrentVelocity += desiredAcceleration * deltaTime;
                m_CurrentVelocity *= Mathf.Pow(m_Drag, deltaTime * 60f);  // Frame-rate independent drag decay
                m_CurrentVelocity = Vector3.ClampMagnitude(m_CurrentVelocity, m_MoveSpeed * 2f);  // Clamp max velocity

                return m_CurrentVelocity * deltaTime;  // MODIFIED: Return velocity-based movement
            }
